...
@property (readwrite, retain, nonatomic) NSMutableArray *hits;
@property (readwrite, retain, nonatomic) AFHTTPClient *httpClient;
    ...
    if (self) {
        ...
        */
        [[NSNotificationCenter defaultCenter] addObserverForName:NSApplicationWillTerminateNotification object:NSApp queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note) {
            //@strongify(self);
            NSLog(@"Application will terminate: %@", note);
...
{
    NSLog(@"[%@] Fire events (size: %@)", [timer fireDate], self.hits);
    [self dispatch];
}
...
- (void)dispatch;
{
    ...
    NSLog(@"Hits = %@", [self.hits description]);
...
//            }
        ...
        [self.httpClient postPath:@"/collect" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            NSLog(@"Fail to submit params: %@ with error %@", params, error);
